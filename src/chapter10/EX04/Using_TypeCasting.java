...
//		B b = (B)aa;		//aa 는 B타입을 내포하고 있지 않으므로 다운 캐스팅을 할 수 없다.
							//구문에는 문제가 없고, 실행시 문제(예외) 발생,
//		C c = (C)aa;		//aa는 C타입을 내포하고 있지 않으므로 다운 캐스팅을 할 수 없다.
		
		//3. 다운 캐스팅이 가능한 경우
		A ab = new B();		//ab는 A, B를 내포하고 있다, A 타입으로 지정됨
		B b = (B)ab;		//A(부모) -- > B(자식)
//		C c = (C)ab;		//ab는 C 타입을 내포하고 있자 않음. 캐스팅이 불가능
							// 구문은 문제 없음. 실행시 오류 발생
		
		B bd = new D();		//bd는 A,B,D를 내포하고 있고, B타입으로 지정
							//bd는 A,B의 필드와 메소드를 접근
		D d = (D)bd;		//bd는 D 를 내포하므로 B->D //다운캐스팅
							//d는 A, B, D의 모든 필드와 메소드를 사용 가능
//		C c = (C)bd;		//bd는 C를 내포하고 있지 않는다. 캐스팅이 불가능
							//구문에는 문제 없음. 실행시 오류
	
		
		A ad = new D();		//ad에는 A,B,D를 포합하고 있다. A타입으로 지정됨
