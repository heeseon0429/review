...
public class Using_Method06 {
	...
	static int div (int[] a ) {
		//돌아온 배열의 모든 값을 빼서 뺀 값을 리턴
		int div = 7;
		for(int i =0; i < a.length; i++) {
			if(i == 0) {continue;}		//i==0일때는 아래 내용 실행하지 않고
			div -= a[i];		//div = div -a[i]
		}
		return div;
	}
	...
	static long mul (int[] a ) {
		//돌아온 배열의 모든 값을 곱해서 곱한값을 리턴
		long mul = 1;			//곱할떄는 초기값을 1로 설정
		for(int i = 0; i < a.length; i++) {
			mul *= a[i]; 		//mul = mul *a[i]
		}
		return mul;
